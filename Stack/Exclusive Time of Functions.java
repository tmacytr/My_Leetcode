/*
	...
	n = 2
	logs = 
	["0:start:0",       //注意这里怎么算execute time: 比如0 ~ 2就是 0, 1, 2 离散时间变量
	 "1:start:2",
	 "1:end:5",
	 "0:end:6"]
	Output:[3, 4]
...
class Solution {
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for (String log : logs) {
            ...
            if (!stack.isEmpty())
            	// parts[2] is the start of next interval, doesn't belong to current interval.
                res[stack.peek()] += Integer.valueOf(parts[2]) - preTime;
            preTime = Integer.valueOf(parts[2]);
            if (parts[1].equals("start"))
                stack.push(Integer.valueOf(parts[0]));
            else {
            	  // parts[2] is end of current interval, belong to current interval. That's why we have +1 here
                res[stack.pop()]++;
                preTime++;
            }
        }
