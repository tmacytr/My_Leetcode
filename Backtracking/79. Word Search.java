...
*/
/*
    这道题分析看，就是一个词，在一行出现也是true，一列出现也是true，一行往下拐弯也是true，一行往上拐弯也是true，一列往左拐弯 也是true，一列往右拐弯也是true。
...
class Solution {
    ...
    public boolean dfs(char[][] board, String word, int index, int rowIndex, int colIndex, boolean[][] visited) {
        if (index == word.length())
            return true;
        //剪枝，各种不符合项
        if (rowIndex < 0 || colIndex < 0 || rowIndex >= board.length || colIndex >= board[0].length || visited[rowIndex][colIndex] == true || board[rowIndex][colIndex] != word.charAt(index))
            return false;
