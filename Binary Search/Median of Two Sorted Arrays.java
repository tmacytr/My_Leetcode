...
public class Solution {
	...
	public class Solution {
	    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
	        ...
	        // 奇数
	        if (len % 2 != 0) {
	            return helper(nums1, 0, m - 1, nums2, 0, n - 1, len / 2 + 1);
	        } else {
	            // 偶数
	            double x = helper(nums1, 0, m - 1, nums2, 0, n - 1, len / 2);
	            double y = helper(nums1, 0, m - 1, nums2, 0, n - 1, len / 2 + 1);
	            return (double)(x + y) / 2;
	        }
	    }
	    
	    public static int helper(int[] nums1, int aStart, int aEnd, int[] nums2, int bStart, int bEnd, int k) {
	        int m = aEnd - aStart + 1;
	        int n = bEnd - bStart + 1;
	        
	        if (m > n)
	            return helper(nums2, bStart, bEnd, nums1, aStart, aEnd, k); //默认 nums1.length < nums2.length,如果大于则颠倒俩数组
	        if (m == 0)
	            return nums2[k - 1];
	        if (n == 0)
	            return nums1[k - 1];
	        ...
	        if (nums1[aStart + partA - 1] < nums2[bStart + partB - 1]) {
	            return helper(nums1, aStart + partA, aEnd, nums2, bStart, bEnd, partB);

	        } else if (nums1[aStart + partA - 1] > nums2[bStart + partB - 1]) {
	            return helper(nums1, aStart, aEnd, nums2, bStart + partB, bEnd, k - partB);
...
// Solution4: https://github.com/Blankj/awesome-java-leetcode/blob/master/note/004/README.md
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
    ...
    private int helper(int[] nums1, int m, int[] nums2, int n, int k) {
        ...
        int partA = m + k / 2 - 1;
        int partB = n + k / 2 - 1;
        int mid1 = partA < nums1.length ? nums1[partA] : Integer.MAX_VALUE;
        int mid2 = partB < nums2.length ? nums2[partB] : Integer.MAX_VALUE;
        if (mid1 < mid2) {
