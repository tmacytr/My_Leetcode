...
//此题就是找规律， magical string的每个字符意味着有多少个1或者2，也就是1，2的次数组成的magic string
/*
    Algorithm:
...
class Solution {
    public int magicalString(int n) {
        ...
        int head = 2, tail = 3, num = 1, res = 1; // head是指向要放1或2的count, tail是要放1和2的位置的指针，num是当前要放1或者2

        while (tail < n) {
            for (int i = 0; i < nums[head] && tail < n; i++) {
                nums[tail++] = num;
                if (num == 1)
                    res++;
            }
            num = num ^ 3; // num = 3 - num, it's using for fliping from 1 and 2
            head++;
        }
        return res;
    }
