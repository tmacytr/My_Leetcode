/*
	...
	see CC150 OO Design for details.
	 ...
	 4) Each row, 
	    motorcycle spots id is in range [0,k/4)(0 is included, k/4 is not included), 
	    compact spots id is in range [k/4,k/4*3) 
	    large spots id is in range [k/4*3,k).
...
class Level {
    ...
    private int findAvailableSpots(Vehicle vehicle) {
        ...
        for (int i = 0; i < spots.length; i++) {
            ...
            //这里判断的作用在于，如果是到了一个新的行，则之前所有的匹配spot数都要清空
    ...
    private boolean parkStartingAtSpot(int spotNumber, Vehicle vehicle) {
        ...
        for (int i = spotNumber; i < spotNumber + vehicle.spotsNeeded; i++) {
            success &= spots[i].park(vehicle);//用与操作来判断相连的spots是否都满足要求的car size
        }
