...
public class Solution {
	 public int minDistance(String word1, String word2) {
	 	...
	 	//dp[i][j] means the minimum convert steps, that in front of length i  character of word1 
	 	//convert to in front of length j character of word2 , 
	 	int[][] dp = new int[len1 + 1][len2 + 1];
	 	
	 	dp[0][0] = 0;

	 	//since the length of word2 equals 0, so the convert number equals the length of word1
	 	for (int i = 1; i <= len1; i++) 
	 		dp[i][0] = i;
	 	for (int j = 1; j <= len2; j++) 
	 		dp[0][j] = j;

	 	//start to caculate from index 1,
	 	for (int i = 1; i <= len1; i++) {
	 		for (int j = 1; j <= len2; j++) {
	 			if (word1.charAt(i - 1) == word2.charAt(j - 1))// means we dont need to do any operation in these two character
	 				dp[i][j] = dp[i - 1][j - 1];
	 			else {
